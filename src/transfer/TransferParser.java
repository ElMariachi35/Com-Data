...
public class TransferParser implements Serializable {
    ...
    public List<RawTransfer> parseTransfers() throws IOException, InterruptedException {
        ...
        for (int i = 0; i < newsList.size(); i++) {
            ...
            String dateString = newsList.get(i).select(".newsheader span").attr("title");
...
//        String newsPage = readFile("C:\\Users\\Gerald\\Desktop\\Comunio\\allTransfers.html", Charset.defaultCharset());
    ...
    private boolean isTransferNews(List<Element> newsList, int i) {
        return newsList.get(i).select(".newsheader").text().contains("Computer > Transfers");
    }
    ...
    private String determineTo(String[] token) {
        ...
        to = StringUtils.join(Arrays.copyOfRange(token, indexOfZu + 1, token.length), " ").trim();
    ...
    private String determineFrom(String[] token) {
        ...
        return StringUtils.join(Arrays.copyOfRange(token, indexOfVon + 1, indexOfZu), " ").trim();
    }
    ...
    private Date determineNewsDate(String dateString) {
        ...
        DateTimeFormatter formatter = DateTimeFormat.forPattern(rawFormatString);
    ...
    private String getSourceOfNewsPage() {
        ...
        ComunioTeamNewsPage teamNewsPage = loginPage.login("Hotboy48", "Thinkp4d");
